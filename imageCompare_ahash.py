...
stream = urllib2.urlopen('http://192.168.0.103:8080//video?x=y')
bytes = ''
s1 = ''
s2 = ''
hashArray = []
...
def isFaceExists(currFaceScore, faceScoreArray):
                ...
                for imageInArray in currentArray:
                    ...
                    for i in range(len(curr_image_score)):
                        if imageInArray[i] != curr_image_score[i]:
                            diff += 1
                    if diff > 50:
                        if isFaceExists(curr_image_score,currentArray) != True:
